 ...
 * as dbap.c, so it drops in behind the bus seam as a selectable alternative (bw_set_panner). The
...
typedef struct {
    float    c[BW_CHANNELS];          /* placement correction (1 / local density) per speaker */
    float    sdir[BW_CHANNELS][3];    /* unit speaker directions from the cached listener */
 ...
 * (caller provides >= BW_CHANNELS). SPCAP assumes a FIXED observer; with a moving (tracked) listener
