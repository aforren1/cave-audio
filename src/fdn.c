...
struct Fdn {
    ...
    float    dcomb[BW_CHANNELS][FDN_N];   /* line -> speaker render (bed decode of each line's plane wave) */
...
Fdn* fdn_create(const Layout* L, uint32_t sample_rate, uint32_t channels) {
    if (!L || sample_rate == 0 || channels == 0 || channels > BW_CHANNELS) return NULL;
    ...
    for (int l = 0; l < FDN_N; ++l) {
        ...
        for (uint32_t s = 0; s < channels; ++s) {
            float acc = 0.f;
            for (int k = 0; k < BW_AMBI_CH; ++k) acc += dec[s][k] * y[k];
            f->dcomb[s][l] = acc;
        }
