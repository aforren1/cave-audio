...
enum { BW_BIQUAD_LOWSHELF = 0, BW_BIQUAD_PEAK = 1, BW_BIQUAD_HIGHSHELF = 2 };
...
static inline void bw_biquad_rbj(int type, double cw0, double alpha, double A, float out[5]) {
    double b0, b1, b2, a0, a1, a2;
    if (type == BW_BIQUAD_PEAK) {
        a0 = 1.0 + alpha / A; a1 = -2.0 * cw0;        a2 = 1.0 - alpha / A;
        b0 = 1.0 + alpha * A; b1 = -2.0 * cw0;        b2 = 1.0 - alpha * A;
    } else {
        double t = 2.0 * sqrt(A) * alpha;
        if (type == BW_BIQUAD_LOWSHELF) {
            a0 =  (A + 1) + (A - 1) * cw0 + t;
            a1 = -2.0 * ((A - 1) + (A + 1) * cw0);
            a2 =  (A + 1) + (A - 1) * cw0 - t;
            b0 =  A * ((A + 1) - (A - 1) * cw0 + t);
...
static inline void bw_biquad_rbj_hz(int type, double fc, double q, double gain_db, double fs, float out[5]) {
    double w0 = 2.0 * M_PI * fc / fs;
    bw_biquad_rbj(type, cos(w0), sin(w0) / (2.0 * q), pow(10.0, gain_db / 40.0), out);
}
