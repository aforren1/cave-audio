...
int allrad_build_decode(const Layout* L, float decode[BWA_CHANNELS][BWA_AMBI_CH]) {
    ...
    float r[BWA_CHANNELS + 2][3];   /* + up to 2 imaginary pole speakers */
    ...
    for (int v = 0; v < ALLRAD_M; ++v) {
        ...
        float row[BWA_AMBI_CH];
        for (int kk = 0; kk < BWA_AMBI_CH; ++kk) {
        ...
        for (int q = 0; q < 3; ++q) {
            ...
            for (int kk = 0; kk < BWA_AMBI_CH; ++kk) decode[spk[q]][kk] += bg[q] * row[kk];
        }
    }
     ...
     * = BWA_AMBI_CH/N  (since sum_m Y_lm^SN3D(dir)^2 = 1, summed over speakers ~ N/(2l+1) each channel). */
    double e_sad = (double)BWA_AMBI_CH / (double)N;
    float scale = (float)sqrt(e_sad / e_all);
    for (uint32_t s = 0; s < N; ++s)
        for (int kk = 0; kk < BWA_AMBI_CH; ++kk) decode[s][kk] *= scale;
