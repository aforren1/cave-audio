...
- **Status: ✅ done.** `layout.c` loads `cave_layout.json` (via cJSON, fetched + pinned)
  ...
  after the mix; the engine loads the layout at `bw_create`. `test_dsp` verifies it: a
...
- **Status: ~done; the by-ear headphone check is the remaining piece.**
  ...
  - **Production HRTF decode, stage 2 (built + running):** `steam_decode.c` is the
    ...
    fallback (`BW_HAVE_STEAMAUDIO`; CMake auto-detects
...
- **Status: ~done (parser + handoff verified; live capture pending hardware).**
  ...
  the audio thread never blocks). With `track_internal = true`, `rt_render` samples
  ...
  handshake (env-overridable; config is via `BWAUDIO_NATNET_*`, see `api.md`).
...
- **Status: ✅ done.** `bindings/unity` is a UPM package (`com.brainworks.bwaudio`).
  ...
  `BwClipAttribute.cs` for the acoustics/bed features. Editor:
  `BwAudioProjectCheck.cs` (warns when Unity's built-in audio is still enabled, with
  one-click disable) and `BwClipDrawer.cs` (the `[BwClip]` StreamingAssets picker).
  The engine's CMake build stages `bwaudio.dll` + `phonon.dll` into
...
- Steam Audio materials — **occlusion + per-band transmission EQ + directivity
  ...
  transmission EQ + a directivity gain on the pre-pan stage). `bw_scene_set_mesh` /
  `bw_source_set_occlusion` / `bw_source_set_directivity`.
- Reflection bed — **implemented** (`src/steam_reflect.c`): a reflections sim →
  ambisonic IR → SH→26 decode, registered as the rt bus tap at `bw_start`. The
  `reflect` ctest proves it's directional. **Baking is in too**: `BWAUDIO_BAKE=1`
...
- Sound pathing — **implemented** (`src/steam_path.c`, opt-in via `BWAUDIO_PATHING`):
...
- Ambisonic bed — **implemented** (`bw_load_ambix` + `bw_bed_*`): a file-fed AmbiX
...
- Audio file formats — **WAV/FLAC/MP3 + resample-on-load implemented**
  (`src/sound.c`, dr_libs).
- `bw_source_create_stream` for procedural/engine-generated audio. Still future —
  only file streaming exists today (`bw_load_sound_streaming`).
...
- **Output protection limiter** — `bw_set_limiter` / `bw_set_limiter_ceiling`, on by
  default at -1 dBFS; linked across channels, the final stage in `rt_render`.
- **Voice priority + click-free steal** — `bw_source_set_priority` (255 = protected);
  a full pool steals the lowest-priority voice through a fade reserve.
- **Pause/seek** — `bw_source_set_paused` / `bw_source_seek`; ramped gates, so both
  are click-free.
- **Disk streaming** — `bw_load_sound_streaming` (`src/stream.c`); a background
  decode thread feeds a per-stream SPSC ring the audio thread pulls from.
- **Dual-band panning** — `bw_set_dual_band`; 700 Hz crossover, amplitude-normalised
  LF / power-normalised HF, live A/B.
- **SPCAP/VBAP** — `bw_set_panner`; fixed-listener alternatives to DBAP
  (`src/spcap.c`, `src/vbap.c`).
- **Source spread** — `bw_source_set_spread`, 0 = point to 1 = wide, panner-agnostic.
- **Doppler + air absorption** — `bw_source_set_doppler` /
  `bw_source_set_air_absorption`; phonon-free per-voice DSP in `src/rt.c`.
- **Calibration suite** — `bw_calibrate` (sweep → trims → acoustic position survey →
  room report; `src/measure.c` / `src/calib.c`), Zylia ZM-1 single-position
  localization (`src/zylia.c`), and the `bw_calib_view` station. See
  `docs/calibration.md`.
- **GUI tools** — `bw_playground`, `bw_calib_view`, `bw_layout_tool`; all on the
