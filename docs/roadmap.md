...
- **Status: ~done; the by-ear headphone check is the remaining piece.**
  ...
  - **Production HRTF decode, stage 2 (built + running):** `steam_decode.c` is the
    ...
    left→left, 180° flips).
  - **Convention** (`test_ambi` versus phonon's hardcoded SH constants): phonon decodes
...
- **Status: ~done (parser + handoff verified; live capture pending hardware).**
  `natnet.c` parses the NatNet **FrameOfData** multicast/unicast stream **off-wire**—the
  SDK is proprietary and would conflict with GPLv3 under distribution, so it is a
...
- Audio file formats: **WAV/FLAC/MP3 + resample-on-load implemented**
  (`src/sound.c`, dr_libs).
- `bwa_source_create_push` for procedural/engine-generated audio, **implemented**:
