...
bed is listener-*centric*: one ambisonic field decoded around one point. Structurally, it cannot
give parallax. Walk toward a wall and the reflection does not change direction. The ISM
...
That configuration never calls `steam_reflect` at all. **Do not run the Steam reflection bed and the
ISM at the same time.** Steam's bed already contains early reflections, so you would hear them twice.
...
automatic occlusion, pathing, and the real HRTF monitor. The monitor is the practical loss for a
developer working off-site. The fallback is a lateral pan: fine for routing checks, useless for
timbre or front/back.
...
([spatialization.md](./spatialization.md)). Materials build out that layer:
...
is a third feed onto the same bus; see "Pathing" below.) The engine decodes the reflection ambisonic field
...
with `bwa_scene_set_dynamic_transform`), a cheap top-level BVH refit (not a geometry rebuild). It is
the acoustic analogue of a physics collider with a transform. Replacing the whole *static* mesh at runtime
...
code path. That access-violates on the odd ambisonic channels (the per-channel FFT stride is
...
that pushes geometry from a Unity/Unreal level **must convert mesh vertices through the same
...
room. Then call `bwa_scene_set_mesh_mat` once, with the box walls plus your geometry. (The Godot
...
ramp. The occlusion gain and the three band gains ramp **per sample** (invariant 4). The engine derives the biquad
coefficient sets once per block from per-band filter prototypes precomputed at create
(`eq_proto`, rate-derived; this runs at 96 kHz too), and interpolates them across the block.
...
+ materials for a single listener-centric bed source. It produces a **single mixed ambisonic IR**
...
which yields an ambisonic reflection signal. It then **decodes that signal to the bus channels with
an `IPLAmbisonicsDecodeEffect` in panning mode** (`binaural = false`), and sums it onto the bus.
That decode's `speakerLayout` is the **same `IPL_SPEAKERLAYOUTTYPE_CUSTOM` layout** the binaural
monitor builds from the surveyed geometry (one direction per layout speaker).
...
irregular-layout decode (for example, AllRAD / energy-preserving max-rE), or you get a
rotated/scaled bed.
...
monitor uses 3rd). That keeps the reflection channel count and the convolution cheap. The hybrid
reverb split described above bounds the convolution cost itself: the convolution runs against a
short early IR, and the long tail is a cheap parametric reverb that shares the same ambisonic→bus
decode (`reverb_estimator` / `hybrid_reverb_estimator` in the SDK).

## Threading and RT-safety
...
- **Audio thread**: applies only the per-block effects (below). The engine allocates all effect/context objects, the
  decode effect, and every FFT/convolution scratch **at `bwa_start`**, never in
  the callback.
...
convolution is **one** instance regardless of source count. The engine does not build per-source reflection
streams. Each would add a convolution. This call is the same one
...
**Fixed-at-create, not live knobs.** The engine creates `IPLReflectionEffect` with a fixed `irSize` and
...
`bwa_create` and `bwa_start`. Changing any of them means `bwa_stop`/`bwa_start`. The one live
...
(for example, 2nd order × 2 s × 48 kHz ≈ 9 × 96000 floats ≈ 3.4 MB), far too large to copy in
...
read state). Ownership solves lifetime, not a pool: `params.ir` aliases interior memory of
the immortal bed `IPLSource`. The engine creates that source once and releases it only at destroy,
after the audio thread has joined, so there is no use-after-free window.

**Per-source results are generation-keyed.** The engine keys occlusion/directivity publishes by the
full generation-counted `bwa_source` handle, and **drops them on a generation mismatch** at consume
time (just as `voice_for` drops stale commands). A slot destroyed and recycled between simulation
and consumption can't apply stale values to a different sound. This path is **decoupled from
...
**No clicks on low-rate updates.** The occlusion gain and the band gains ramp **per sample**
(invariant 4). The engine rebuilds the filter coefficients once per block (above).
...
To keep the bus seam, reflections go **ambisonic → speaker decode → bus**. The binaural monitor
...
audition the identical bus" guarantee. **Do not** short-circuit the monitor by feeding it the pre-decode
reflection ambisonics directly. That would create a second, bus-bypassing binaural
consumer and desync the array-versus-monitor reflection content.

## Geometric default, perceptual option

Everything above is the **geometric** path (materials + scene → ray-traced reflections), and it
is the **default**. For content with no usable mesh (abstract or musical), the *perceptual*
school (IRCAM Spat) drives a parametric reverb from listener-tested knobs instead of geometry:
presence, warmth / brillance, room presence, reverberance, envelopment. The hybrid reverb's FDN
late tail already exposes what such a mapping targets, so a perceptual mode is a control mapping
(**no new DSP**), same bus, mutually exclusive with geometry *per source*. **Not implemented.**

## The API surface

The material and reflection calls are **additive**: an engine with no scene renders exactly as if
materials didn't exist. [api.md](./api.md) documents the full surface with per-call threading semantics
("Materials and scene geometry", "Occlusion and directivity", "Reflection
bed"). Signatures live in [include/bw_audio.h](../include/bw_audio.h).

One design boundary worth stating here: **there is no scene-file loader** (`bwa_scene_load` does
not exist). A binding pushes room-space geometry and materials through the setters at load time.
The game owns the scene ([integration.md](./integration.md)). A standalone scene JSON, and the
perceptual reverb mode above, are both future work.
...
One more is simply future work: the
**perceptual reverb mode** (above). Doppler and air absorption are phonon-free per-voice DSP
...
listener gets +X-biased reverb out of baked data (ratio ≈ 1.4, versus ~1.0 for an omni bed).
...
- **Bake (Stage 1, `steam_path_create`).** A probe grid spans the layout (+margin) at mean speaker
  height. It uses the SAME OBB-transform convention learned for reflections (center in the translation
...
phonon's `kMaxEQGain` (0.0625)), exactly phonon's `normalizeEQ` mode. It publishes that tilt beside
the shCoeffs (`rt_set_pathing`, same handle-gated double buffer). The mixer applies that tilt as the
same low-shelf/peak/high-shelf biquad cascade the occlusion EQ uses, to the **un-occluded** `s_raw`
*before* the SH-encode. That is precisely phonon's own `path_effect.cpp` render order (EQ the mono
signal, then scale each SH channel). It's ramped per sample (invariant 4) and bypassed while flat, so a
...
**acoustically treat the physical room (make it absorptive)** so the rendered virtual
