...
##
## Layout: the first non-flag user argument (`-- my_layout.json`), else a cave_layout.json beside
## the executable or in the project root, else the 24-speaker dome shipped in this folder
## (dome_24.json, generated by tools/layout/gen_dome.py), else the engine's built-in grid.
## --selftest always takes the dome, so the suite never depends on a machine-local file.
##
...
# The default array. The core opens files by OS path and an exported .pck has none for res://,
# so the dome is copied out to user:// once and the engine is pointed at the copy.
const DOME_RES := "res://addons/bw_audio/playground/dome_24.json"
const DOME_USER := "user://bwa_playground_dome_24.json"
...
var head := Vector3.ZERO          # the engine's own listening point: listening_point_m, else the centroid
var head_yaw := 0.0
...
var backend := "none"
var layout_path := ""            # resolved once in _ready; "" = the engine's built-in grid
var layout_is_dome := false
...
## The same order as the C++ playground: an argument, a surveyed cave_layout.json, the shipped
## dome, the engine's grid.
	...
	if not _headless:
		for a in OS.get_cmdline_user_args():
			if not a.begins_with("-"):
				layout_path = a
				return
		for dir in [OS.get_executable_path().get_base_dir(), ProjectSettings.globalize_path("res://")]:
			var p: String = dir.path_join("cave_layout.json")
			if FileAccess.file_exists(p):
				layout_path = p
				return
	var bytes := FileAccess.get_file_as_bytes(DOME_RES)
	if bytes.is_empty():
		push_warning("playground: %s is missing; running the engine's built-in grid" % DOME_RES)
		return
	var f := FileAccess.open(DOME_USER, FileAccess.WRITE)
	if f == null:
		push_warning("playground: cannot write %s; running the engine's built-in grid" % DOME_USER)
		return
	f.store_buffer(bytes)
	f.close()
	layout_path = DOME_USER
	layout_is_dome = true
...
func _build_rig(mode: int) -> void:
	...
	engine.layout_path = layout_path
	...
	# the array centroid), not the room origin. A fresh engine's listener sits there until the
	# listener node moves it, so the pose readback IS that point.
	head = engine.get_listener_transform().origin
	...
	print("playground: %s, %d speakers, layout %s" % [backend, nspk,
		("the shipped dome (%s)" % DOME_RES) if layout_is_dome
		else (layout_path if not layout_path.is_empty() else "engine default grid")])
...
func _run_selftest() -> void:
	var fail := 0
	# The suite runs on the shipped dome: 24 speakers, none below the floor, all 2 m from
	# (0, 1.5, 0). The head sits at the file's declared listening_point_m, the sphere center,
	# not the centroid (which the floor cut lifts to ~1.75 m).
	if not layout_is_dome or nspk != 24 or engine.get_channel_count() != 24:
		push_error("playground: not on the shipped dome (dome=%s, %d speakers, %d channels)"
			% [layout_is_dome, nspk, engine.get_channel_count()])
		fail += 1
	for k in nspk:
		var r := speakers[k].distance_to(Vector3(0, 1.5, 0))
		if speakers[k].y < 0.0 or absf(r - 2.0) > 1e-3:
			push_error("playground: dome speaker %d at %s (radius %.4f)" % [k, speakers[k], r])
			fail += 1
			break
	if head.distance_to(Vector3(0, 1.5, 0)) > 1e-3:
		push_error("playground: head at %s, expected the dome's listening point (0, 1.5, 0)" % head)
		fail += 1
